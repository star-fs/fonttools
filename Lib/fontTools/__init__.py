...
class TTFont:
	...
	def __init__(self, file=None, res_name_or_index=None,
			...
			verbose=1, recalcBBoxes=1, allowVID=0, ignoreDecompileErrors=False,
			fontNumber=-1, type="TTF"):
		...
		if type == "WOFF":
			self.reader = woff.WOFFReader(file, checkChecksums)
			self.sfntVersion = self.reader.wOFFflavor
		elif type == "EOT":
			self.reader = eot.EOTReader(file, checkChecksums)
			self.sfntVersion = sfntVersion
		else:
	...
	def saveWoff(self, file):
		...
		numTables = len(tags)
		tmp = file
		writer = sfnt.SFNTWoFFWriter(tmp, numTables, self.sfntVersion)
		done = []
		for tag in tags:
			self._writeTable(tag, writer, done)
	...
	def saveEot(self, file):
		...
		numTables = len(tags)
		tmp = file
		writer = sfnt.SFNTEotWriter(tmp, numTables, self.sfntVersion)
		done = []
		for tag in tags:
			self._writeTable(tag, writer, done)
	...
	def getGlyphOrder(self):
		...
		if self.has_key('CFF '):
			...
			glyphOrder = None
                        table = self['post']
			glyphOrder = table.getGlyphOrder()
