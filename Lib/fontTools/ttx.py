...
def ttList(input, output, options, type):
	import string
	ttf = TTFont(input, fontNumber=options.fontNumber, type=type)
	reader = ttf.reader
	tags = reader.keys()
	tags.sort()
...
def ttCompile(input, output, options, type):
	print 'Compiling "%s" to "%s"...' % (input, output)
	ttf = TTFont(options.mergeFile,
			recalcBBoxes=options.recalcBBoxes,
			verbose=options.verbose, allowVID=options.allowVID, type=type)
	ttf.importXML(input)
	try:
		ttf.save(output)
	...
	for input in files:
		...
		if tp in ("OTF", "TTF", "TTC", "WOFF", "EOT"):
			...
			action = ttCompile
                        ...
                        print 'Unknown file type: "%s" - "%s"' % (tp, input)
			continue
		
		output = makeOutputFileName(input, options.outputDir, extension)
		jobs.append((action, input, output, tp))
	return jobs, options


def process(jobs, options):
	for action, input, output, type in jobs:
		action(input, output, options, type)
